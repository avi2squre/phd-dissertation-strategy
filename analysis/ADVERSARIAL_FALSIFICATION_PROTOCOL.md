...
- **Authority if approved:** canonical process specification for adversarial scientific review of candidate dissertation directions

This document defines the common scientific rules, evidentiary standards, search methodology, decision semantics, and information-flow constraints for adversarial falsification in this repository. It standardizes the analytical instrument used by independent streams without prescribing their conclusions.
...
The purpose of adversarial falsification is to expose fatal or material weaknesses before substantial research time is committed. The protocol directs analysis toward evidence that could change the scientific status of a candidate rather than toward ceremonial criticism or supportive literature accumulation.

The optimization target is:

> Maximize decision-relevant scientific uncertainty reduction per unit of researcher attention, model computation, source-reading effort, annotation effort, and implementation cost.
...
A successful falsification phase may eliminate an attractive candidate, require material revision, or leave it unresolved. Finding that a candidate fails is a useful outcome because it prevents larger downstream losses.
...
4. [`round3_normalization_reconciliation.md`](round3_normalization_reconciliation.md), which freezes the eight normalized candidate dissertation directions that enter adversarial falsification.
...
### 3.3 Non-goals during scientific phases
...
A change is not merely material revision when it replaces the candidate’s central object or thesis. That creates a redefined candidate and requires a human checkpoint.
...
### 6.2 Steelman before rejecting

A candidate should be evaluated in its strongest coherent formulation supported by the frozen record. The reviewer may clarify ambiguity but may not invent a superior new candidate and then credit the original with surviving.

### 6.3 One thesis, multiple tests
...
### 6.4 Search failure is not evidence of nonexistence
...
### 6.5 Source reuse does not imply interpretation inheritance
...
### 6.6 Scientific symmetry does not require equal conclusions

All streams should use the same definitions, evidence rules, kill tests, output schema, and transition criteria. They are expected to disagree where their searches or reasoning differ.

### 6.7 Equal candidate status does not require equal computation
...
> The eight normalized candidates were frozen in [`round3_normalization_reconciliation.md`](round3_normalization_reconciliation.md) at the completed Day 1 checkpoint.
...
Recent preprints with sufficient methodological detail to evaluate the contribution. They can pose real novelty collisions, but publication status and version date must be recorded.
...
Before searching, decompose the candidate into a search concept map.
...
```text
diagnostic-state trajectory
latent diagnosis-state sequence
diagnosis-status transition
longitudinal clinical belief state
temporal diagnostic evolution
```
...
## 10. Search audit trail and saturation stopping rule
...
A search may be considered saturated for the current phase only when:

1. all applicable search layers have been attempted;
2. direct synonyms and concept-level synonyms have both been used;
3. the most dangerous apparent collisions have been inspected beyond metadata;
...
Failure to reach saturation does not invalidate the artifact. It requires an explicit `UNRESOLVED` label and a concrete next-evidence request.
...
### 11.2 Stage B — Contribution-overlap matrix
...
### 11.4 Default fatal-collision condition
...
**Failure condition:** a simple baseline provides essentially equivalent scientific and empirical performance under realistic controls.
...
**Required evidence:** dependency graph covering data, ground truth, annotation, compute, software infrastructure, expertise, governance, collaborators, and time.

Dependencies should be labeled:

- **GREEN:** available or under direct control;
- **YELLOW:** realistically obtainable with a credible path;
- **RED:** uncertain, externally controlled, or potentially fatal.

**Failure condition:** the central claim depends on an uncontrolled and unlikely resource with no scientifically valid fallback.
...
All three streams independently evaluate all eight directions using the six kill tests and two stress tests.
...
- each candidate receives the minimum required scan;
...
**Transition gate:** all three artifacts are frozen, source lists resolve, and no stream has seen another’s Phase 1 conclusions.
...
Not every candidate receives equal additional computation.

**Transition gate:** targeted tasks reach a stopping condition, update the relevant kill tests, and identify candidates requiring cross-examination.
...
- exact candidate version and repository commit must be recorded;
...
8. no blind stream independently assigns a new permanent `LIT-###` identifier;
9. internal project-state claims cite the relevant artifact or commit;
10. no restricted or private information is exposed.
...
| Field | Purpose |
|---|---|
| Source identifier | Permanent, provisional, restricted, or internal provenance ID |
| Full citation or source-record link | Human-readable source identity |
| Stable identifier or link | DOI, PMID, proceedings ID, arXiv, or official URL |
...
If approved, this document should be:

1. linked from `analysis/README.md`;
2. adopted through a new accepted decision in `decisions/decision_log.md`;
3. treated as canonical for Phases 1 through 5 and the Phase 6 handoff;
4. changed materially only with user approval and a corresponding decision-log update.
...
- **v1.0:** first approved canonical version.

Material changes alter definitions, decision thresholds, information flow, scientific outcomes, or phase gates. Editorial clarifications that preserve meaning may be recorded as metadata-only or wording-only revisions, with explicit provenance.
...
`normalized candidate -> layered search -> collision assessment -> six kill tests -> two stress tests -> cheapest discriminating test -> next evidence required -> independent phase outcome -> reconciliation -> deep falsification -> cross-examination -> scientific survivor freeze -> strategic optimization`

The governing rule is:

> Standardize the scientific instrument, preserve independent reasoning, cite every decision-relevant source-derived claim, and stop only when additional work is unlikely to change the scientific decision or next action.

**STATUS: DRAFT — USER REVIEW REQUIRED. No analysis stream is authorized to begin Phase 1 solely on the basis of this draft.**